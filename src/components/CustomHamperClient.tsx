...
export default function CustomHamperClient({ products }: { products: any[] }) {
  ...
  const categories = ["all", ...Array.from(new Set(products.map((p) => p.type === "freshly_bakes" ? "Freshly Baked" : "Regular")))];

  const filtered = products.filter((p) => {
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase());
    const matchCat = activeCategory === "all" || (activeCategory === "Freshly Baked" ? p.type === "freshly_bakes" : p.type === "regular");
  ...
  const buildWhatsAppMessage = () => {
    const itemsList = hamperItems.map((i) => `• ${i.name} x${i.qty} — ₹${((i.discount_price || i.price) * i.qty).toLocaleString("en-IN")}`).join("\n");
    return encodeURIComponent(`Hi Zaro Bakehouse! I'd like to create a custom hamper:\n\n${itemsList}\n\nTotal: ₹${total.toLocaleString("en-IN")}\n\nPlease confirm availability and packaging options.`);
  };
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border border-border text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 bg-white"
            />
            <div className="flex gap-2">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition-colors capitalize ${activeCategory === cat ? "bg-forest-800 text-white" : "bg-white border border-border hover:bg-accent"}`}>
                  {cat}
                </button>
              ))}
            </div>
              ...
              return (
                <div key={product.id} className={`bg-white rounded-2xl border overflow-hidden transition-all ${qty > 0 ? "border-brand-400 shadow-card" : "border-border"}`}>
                  <div className="aspect-square bg-cream-200 overflow-hidden relative">
                    ...
                    {qty > 0 && (
                      <div className="absolute top-2 right-2 bg-brand-500 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">
                        {qty}
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-3xl border border-border p-6 sticky top-24">
            ...
            {hamperItems.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-muted-foreground text-xs mt-1">Pick items from the left to build your hamper.</p>
              </div>
            ) : (
              <>
                ...
                <div className="border-t border-border pt-4 mb-5">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Final price includes packaging. Confirmed via WhatsApp.</p>
                </div>
                ...
                >
                  ...
                  Send Hamper Request via WhatsApp
                </a>
                <p className="text-xs text-muted-foreground text-center mt-3">We'll confirm availability and delivery details within a few hours.</p>
              </>
            )}
          </div>
        </div>
