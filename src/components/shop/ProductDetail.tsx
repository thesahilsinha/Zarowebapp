...
import {
  Star, Heart, ShoppingBag, Zap, Truck,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div className="space-y-4">
              ...
              {images.map((img, i) => (
                <button key={i} onClick={() => setSelectedImage(i)}
                  className={`w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 transition-all ${
                    selectedImage === i ? "ring-2 ring-brand-500 ring-offset-2" : "opacity-60 hover:opacity-100"
                  }`}>
          ...
          {(product as any).video_url && (
            <div className="rounded-2xl overflow-hidden bg-black aspect-video mt-2">
              {(product as any).video_url.includes("youtube.com") || (product as any).video_url.includes("youtu.be") ? (
                <iframe
                  src={`https://www.youtube.com/embed/${
                    (product as any).video_url.includes("youtu.be")
                      ? (product as any).video_url.split("youtu.be/")[1]?.split("?")[0]
                      : new URLSearchParams((product as any).video_url.split("?")[1]).get("v")
                  }?rel=0`}
                ...
                <video src={(product as any).video_url} controls className="w-full h-full object-cover" />
              )}
            </div>
          )}
          {/* ── END VIDEO SECTION ── */}
        ...
        <div className="flex flex-col">
          ...
          <h1 className="font-playfair text-3xl md:text-4xl font-bold text-foreground mb-3">{product.name}</h1>

          {/* Rating */}
          <div className="flex items-center gap-2 mb-5">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star key={star} size={16} className={
            ...
            <span className="text-sm text-muted-foreground">{product.rating} ({product.rating_count} reviews)</span>
          </div>

          {/* Price */}
          <div className="flex items-baseline gap-3 mb-6">
            <span className="font-bold text-3xl text-foreground">{formatPrice(product.discount_price || product.price)}</span>
            {product.discount_price && (
              <span className="text-muted-foreground text-lg line-through">{formatPrice(product.price)}</span>
            )}
            {discount > 0 && (
          ...
          <div className="bg-green-50 border border-green-100 rounded-2xl p-4 mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-sm font-semibold text-green-800">Estimated Delivery: {deliveryEstimate}</p>
          ...
          {product.stock <= 10 && product.stock > 0 && (
            <p className="text-orange-600 text-sm font-medium mb-4">Only {product.stock} left in stock</p>
          )}
          {product.stock === 0 && (
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <button onClick={handleWishlist}
              className={`w-11 h-11 rounded-xl border flex items-center justify-center transition-all ${
                isWishlisted ? "bg-red-50 border-red-200 text-red-500" : "border-border hover:border-red-200 hover:bg-red-50 hover:text-red-500"
              }`}>
